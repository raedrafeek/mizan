...
| c6eca4e | **Full-app audit**: `scripts/audit-math.ts` (independent recompute of balances/net-worth/cashflow/category-spend/campaign-pct vs API — all pass; keep as regression tool). FIXED: editing a negative adjustment flipped its sign (PATCH now preserves direction; sheet explains it). Known documented gaps: deleting a Buy/Sell txn doesn't revert holding qty; auto-post catch-up posts once per cron run; desktop QuickLog lacks refund/split (use /log) |
